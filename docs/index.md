...
- **Language support**: the default user interface is in English, but [some
  ...
  Kerko](guides.md#translating-kerko). Also to consider: locales supported by
  the [Zotero Data Schema][Zotero_schema] (which provides the names of fields,
  item types and author types displayed by Kerko); languages supported by Whoosh
  (which provides the search capabilities), i.e., ar, da, nl, en, fi, fr, de,
...
- **Relations**: bibliographic record pages show links to related items, if any.
  ...
  through notes (see [
  guide](guides.md#providing-cites-and-cited-by-relations)). Custom applications
...
- **Integration**: as a Flask [blueprint][Flask_blueprint], Kerko can be
  ...
  KerkoApp](getting_started.md#getting-started-with-kerkoapp).

...
from whoosh.fields import ID, NUMERIC, STORED, FieldConfigurationError, Schema, UnknownFieldError
...
def sync_cache(full=False):
    ...
    library_context = zotero.request_library_context(
        zotero_credentials
        ...
        for item_key in fulltext_items.keys():
            with cache.searcher() as searcher:
                results = searcher.search(
                    QueryParser("key", schema=cache.schema, plugins=[]).parse(item_key), limit=1
                )
                if results:
                    count += 1
